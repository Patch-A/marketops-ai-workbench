...
| M1-01, active | The asynchronous HTTP import path, hashed Linux runtime, PostgreSQL 18.4 migration, role/grant attestation, RLS isolation, concurrent idempotency, normal same-container restart recovery, uncommitted connection-loss rollback, and isolated application-level logical backup/restore passed their recorded local and GitHub CI checks through run `31317614862`. | Power-loss durability, durable host volumes, WAL/PITR, production recovery or RPO/RTO, orphan cleanup, browser server-source recovery, production reliability, or M1-01 acceptance. | Complete the separately bounded orphan-cleanup and browser-cutover experiments in the M1-01 runtime integration plan. |
