...
| M1-01, active | The asynchronous HTTP import path, hashed Linux runtime, PostgreSQL 18.4 migration, role/grant attestation, RLS isolation, and concurrent idempotency passed their recorded local and GitHub CI checks through run `31313732611`. | Restart recovery, backup/restore, orphan cleanup, browser server-source recovery, production reliability, or M1-01 acceptance. | Complete the separately bounded restart/connection-loss, backup/restore, cleanup, and browser-cutover experiments in the M1-01 runtime integration plan. |
