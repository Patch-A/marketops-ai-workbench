...
| M1-01, active | The asynchronous HTTP import path, hashed Linux runtime, PostgreSQL 18.4 migration, role/grant attestation, RLS isolation, concurrent idempotency, normal same-container restart recovery, and uncommitted connection-loss rollback passed their recorded local and GitHub CI checks through run `31315869043`. | Power-loss durability, durable host volumes, backup/restore, orphan cleanup, browser server-source recovery, production reliability, or M1-01 acceptance. | Complete the separately bounded backup/restore, cleanup, and browser-cutover experiments in the M1-01 runtime integration plan. |
