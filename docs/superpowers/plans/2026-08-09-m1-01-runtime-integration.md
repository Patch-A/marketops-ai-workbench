...
| Shared unknown | The fixed Python runtime, migration, SQL/RLS behavior, normal same-container restart, uncommitted connection-loss rollback, and isolated application-level logical backup/restore have passed their recorded CI experiments. Power-loss and durable-volume behavior, orphan cleanup, browser cutover, and production reliability remain untested. | Preserve the completed runtime, restart, and backup/restore evidence; run separately bounded experiments for cleanup and browser cutover. Retain commands, hashes, response samples, row snapshots, and failure codes without expanding the proven claim boundary. |
...
- [ ] Test a dry-run-first, race-safe orphan cleanup policy.
