...
(function bootstrapModelCenter(globalScope) {
  ...
  function createModelCenter({ api, root, onToast = () => {}, onIcons = () => {} }) {
    const state = { profiles: [], selectedId: null, editing: false, loading: false, saving: false, saveError: '', probeStatus: '', probeStatusKind: '', match: null };
    ...
    function renderEditor() {
      ...
      if (profile) {
        ...
        probe.addEventListener('click', async () => {
          probe.disabled = true; state.probeStatus = '正在检查模型连接...'; state.probeStatusKind = 'is-working'; setStatus(state.probeStatus, 'loading');
          const inlineStatus = editor.querySelector('#modelProbeStatus');
          if (inlineStatus) { inlineStatus.textContent = state.probeStatus; inlineStatus.className = `form-status ${state.probeStatusKind}`; }
          try {
            ...
            state.probeStatus = checked.health === 'healthy' ? '连接正常。' : (checked.lastError || '模型连接不可用。');
            state.probeStatusKind = checked.health === 'healthy' ? 'is-working' : 'is-error';
            setStatus(state.probeStatus, checked.health === 'healthy' ? 'ready' : 'error');
          } catch (error) { state.probeStatus = error.message || '模型连接检查失败。'; state.probeStatusKind = 'is-error'; setStatus(state.probeStatus, 'error'); }
